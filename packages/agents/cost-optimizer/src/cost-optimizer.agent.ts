...
export class CostOptimizerAgent extends BaseAgent {
  ...
  private async handleCostAnalysis(
    ...
    try {
      ...
      const totalCurrentCost = resources.reduce(
        (sum, r) => sum + (r.allocatedCapacity * r.costPerUnitPerMonth), 0
      );
      ...
      // 5. Publish Optimization Opportunity Event if savings > threshold
      if (totalSavings > 50) {
      ...
      await this.audit.commit({
        ...
        data: {
          reportId: report.reportId,
          savings: totalSavings,
          savings_pct: report.savingsPercentage
        }
      });
